...
FloatingTextManager::~FloatingTextManager() { stopAllDynamicTextUpdates(); }
...
FloatingTextManager::getDynamicTextContent(const std::string& name, const FloatingTextData& data, Player* player) {
    ...
    if (paService) {
        if (player) {
            ...
            return paService->replace(baseText, &playerCtx);
        } else {
...
ll::coro::CoroTask<> FloatingTextManager::updateDynamicTextTask(
    ...
    while (runningFlag) {
        ...
        if (!debugTexts.contains(name)) {
            logger.warn("Dynamic text '{}' not found in Entry::mDebugTexts, stopping update task.", name);
            break; // 如果 DebugText 不存在，则停止此任务
        }
        auto& debugText = debugTexts[name];
        ...
        if (level) {
            level->forEachPlayer([&](Player& player) {
                ...
                if (debugText->getText().value_or("") != newText) { // 避免不必要的更新
                    debugText->setText(newText);
                    // 重新绘制以使更改生效，针对特定玩家
                    debug_shape::DebugShapeDrawer::drawShape(debugText.get(), player);
                }
                return true; // 继续遍历
            });
        } else {
            ...
            if (debugText->getText().value_or("") != newText) { // 避免不必要的更新
                debugText->setText(newText);
                // 重新绘制以使更改生效
                debug_shape::DebugShapeDrawer::drawShape(debugText.get());
